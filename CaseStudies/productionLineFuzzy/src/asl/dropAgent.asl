...
@cpk1 [atomic]	        
+!checkProductStatus : productReady(false) <- .print("2 -Product State"); productState; ?productReady(Y); ?dropButtonStatus(DBS) .print("***BP- Product State =",Y,"***",DBS); .wait(100); !checkProductStatus.      
@cpk2 [atomic]	  
+!checkProductStatus : dropButtonStatus(true) & productReady(true)<- .print("3"); .print(" 3-) Drop process Runs  \n"); conveyorStart; -+conveyorStart(true); shredStart; -+shredStart(true); .send([shredAgent,sortAgent],tell,dropped(true)); !motorDrop.
@cpk3 [atomic]	  
