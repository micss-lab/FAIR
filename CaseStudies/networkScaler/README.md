...
9 rules (fuzzy plans) are written for both agents.  On the boolean side, scale factors assigned to each plan are fixed. Considering the response time, these scale factors are arranged on the fuzzy side by multiplying the membership degrees, having a smooth scaling. Response time is calculated by total workload/numberOfMachines.
